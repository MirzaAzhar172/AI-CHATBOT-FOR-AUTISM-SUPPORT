...
with st.sidebar:
    ...
    with col1:
        if st.button(f"+ {get_text('new_chat')}", use_container_width=True):
            ...
            st.rerun()
    with col2:
        if st.button("𖣠 " + get_language_display(), use_container_width=True):
            langs = ['en', 'ms', 'zh']
            ...
            st.markdown(f"""
                <div style='background-color: {input_bg}; 
                    ...
                    <p style='margin: 0; font-size: 0.85rem; color: {text_color};'>
                        <strong>👶 {current_user.get('child_name', 'Not set')}</strong>
                        {', ' + current_user.get('child_age', '') if current_user.get('child_age') else ''}
                    </p>
                </div>
            """, unsafe_allow_html=True)
        else:
            st.info("👆 Set up profile to get personalized advice!")
    ...
    dark_mode_on = st.toggle(
        f"{'◐ ' + get_text('dark_mode') if st.session_state.dark_mode else '◯ ' + get_text('light_mode')}",
